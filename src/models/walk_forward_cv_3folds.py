...
def main():
    ...
    agg = {}
    metrics_list = ["rmse","mae","r2","bt_sharpe","hit_ratio",
    ...
    print(f"\n  {'Model':<12} {'RMSE':>9} {'+-':>6} {'MAE':>9} {'+-':>6} "
          f"{'R2':>8} {'Sharpe':>9} {'+-':>6} {'Hit':>8}")
    print(f"  {'-'*85}")
    for name in model_names:
        a = agg[name]
        print(f"  {name:<12} "
              f"{a['rmse_mean']:>9.4f} {a['rmse_std']:>6.4f} "
              f"{a['mae_mean']:>9.4f} {a['mae_std']:>6.4f} "
              f"{a['r2_mean']:>8.4f} "
              f"{a['bt_sharpe_mean']:>9.4f} {a['bt_sharpe_std']:>6.4f} "
              f"{a['hit_ratio_mean']:>8.4f}")

    # Ablation
    print(f"\n  Ablation: SC-STGCN vs V-STGCN")
    for m in ["rmse","mae","r2","bt_sharpe"]:
    ...
    for name in model_names:
        ...
        rows.append({
            ...
            "Sharpe_mean": a["bt_sharpe_mean"],
            "Sharpe_std":  a["bt_sharpe_std"],
            "Hit_mean":  a["hit_ratio_mean"],
            "Hit_std":   a["hit_ratio_std"],
        })
