"""
src/models/degree_walk_forward_cv.py
======================================
Walk-Forward CV — data/top50_degree/ (50/50 baglantili, degree sirasi)
"""
...
def main():
    ...
    SEP="="*65
    print(SEP)
    print("  Degree Walk-Forward CV")
    print("  top50_degree: 50/50 baglantili, hem sup hem cus, kurt<100")
    ...
    print(f"\n{SEP}")
    print("  DEGREE CV RESULTS (2 FOLDS)")
    print("  top50_degree: 50/50 baglantili | density=0.159 | kurt=32")
    print(SEP)
    ...
    # Eski top-50 karsilastirmasi
    print(f"\n  Karsilastirma — Eski Top-50 SC-STGCN vs Degree-50 SC-STGCN:")
    ref={"rmse":2.872,"r2":0.040,"bt_sharpe":2.433,"f1":0.500}
    for m,old_v in ref.items():
        new_v=agg["SC-STGCN"][f"{m}_mean"]
        diff=new_v-old_v
        arrow="↑" if (m in ["r2","bt_sharpe","f1"] and diff>0) or \
                      (m=="rmse" and diff<0) else "↓"
        print(f"    {m:<12}: Eski={old_v:>+8.4f}  Yeni={new_v:>+8.4f}  {arrow}{abs(diff):.4f}")
    ...
    pd.DataFrame(rows).to_csv(METRICS_DIR/"degree_walk_forward_cv.csv",index=False)
    print(f"\n  [OK] degree_walk_forward_cv.csv")
    print(f"\n{SEP}  TAMAMLANDI\n{SEP}")
